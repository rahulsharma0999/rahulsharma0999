...
class RequestManagementController extends Controller
{
    ...
    public function pastRequestDetail(Request $request)
    {
       ...
       return view('admin.past-request-detail',['request'=>$request_detail,'service_vans'=>$service_vans,'review'=>$get_review  ,'add_on_services'=> $get_add_on_services]);
    }
    ...
    public function acceptRequest_old(Request $request)
    {
       ...
       DB::table('requests')->where(['id'=>$request->request_id])->update($update_data);
        ...
        //$message = 'Your service request was accepted.';
       // 4-12-19
	   //$message = "Your service request has been confirmed and scheduled. In the app, this will appear under 'Current' or 'Upcoming' tabs.";
	   $message = "Your request has been confirmed and scheduled. On the app, this will appear on ‘Upcoming’";
	...
	//old message  $van_message='Admin is allocated you a new job.';
      //$van_notfy_message = array('sound' =>1,'message'=>'Admin is allocated you a new job.',
      $van_message='Admin has allocated you a new job. Job Id #'.$get_request->id;
      $van_notfy_message = array('sound' =>1,'message'=>'Admin has allocated you a new job. Job Id #'.$get_request->id,
    ...
    {
      $get_request =  DB::table('requests')->where(['id'=>$request->request_id])->first();
        $message='Your car service request was declined.';
        $notfy_key=4;
     	$service=$this->getServiceDetail($request->request_id);
      
      $notfy_message = array('sound' =>1,'message'=>'Your car service request was declined.',
