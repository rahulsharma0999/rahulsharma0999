...
class UserManagementController extends Controller
{
     ...
     public function usersList(Request $request)
    {
	  ...
	  //  $users = User::where("email","NOT LIKE","%yopmail%")
	  // ->where(['status'=>1, 'role'=>2])->orderBy('id','DESC')->get();
    ...
    public function sendContactUs(Request $request){
        ...
        $data = array('name'=>$name,"email" => $email,"message_send" => $message,"logo" => url('public/admin/assets/img/logo-2.png'));
        try{
              Mail::send('contact-us', $data, function($message1) use($email,$message,$name) {
                 $message1->to('support@pw.co.ke', 'Contact us')->subject
                    ("Contact Us");
